...
> _"A prompt is not just a simple question: it's any instruction you give Claude, with or without tools, simple or complex."_
...
> _"A Skill is like an expert playbook that Claude consults automatically when the task is relevant, or that you invoke with a slash-command."_
...
> _"A subagent is a specialist summoned by the main agent for a specific task: it works independently in its own context and only returns the result."_
...
> _"A Claude Code session is a complete agent: it reasons, acts, observes the result, and repeats until the goal is completed."_
...
> _Additional note:_ Both the runtime session and the SDK fully support **[Hooks](https://code.claude.com/docs/en/agent-sdk/hooks)**, configured in `.claude/settings.json`, to intercept key actions (such as conditionally tracking or blocking destructive Bash commands).
...
> _"A Claude Code team: a team lead coordinates; teammates work in parallel, can talk to each other directly, and each has their own context."_
...
_When to use Skills vs Subagents?_ Official docs say: use Skills for reusable prompts/workflows in main context; use Subagents for context isolation and restricted tools.
